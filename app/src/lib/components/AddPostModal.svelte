...
>
	<div class="bg-white border p-4 shadow-lg m-8 lg:w-[60%] w-[80%] h-[60%] relative flex flex-col">
		<h2>{formData.name || 'New podcast'}</h2>
		<div class="flex flex-col my-4 gap-4">
			<label class="label">
				<span>Name: </span>
				<input type="url" class="input p-2" placeholder="Feed title" bind:value={formData.name} />
			</label>
			<label class="label">
				<span>Feed URL: </span>
				<input
